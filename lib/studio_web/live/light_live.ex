defmodule StudioWeb.LightLive do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok, assign(socket, brilho: 10, unit: 0, workers: get_workers())}
  end
  ...
  def handle_event("on", _algo, socket) do
    socket = assign(socket, :brilho, 100)

    {:noreply, socket}
  end

  def handle_event("off", _algo, socket) do
    socket = assign(socket, :brilho, 0)

    {:noreply, socket}
  end

  def handle_event("up", _algo, socket) do
